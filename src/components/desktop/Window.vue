...
export default {
  ...
  computed: {
    desktopBounds() {
      const desktop =
        this.$parent && this.$parent.$refs && this.$parent.$refs.desktop
      if (desktop) {
        return {
    ...
    maxW() {
      return this.desktopBounds.width
    },
    maxH() {
      return this.desktopBounds.height
    },
    window: () => window,
    preserveEmbedAppearance() {
      return Boolean(this.embed && this.embed.includes('brew.build'))
    },
  },
  mounted() {
    this.exit = false
...
<template>
  <vue-resizable ref="resizableComponent" class="resizable" :class="`
      ...
      ${preserveEmbedAppearance ? 'embed-preserve' : ''}
    `" :style="`
