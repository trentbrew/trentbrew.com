  ...
  <main>
    <div>
      ...
      <div class="root" :style="clickActive && 'transform: scale(0.95)'">
        <div v-if="isMobile" class="mobile-veil absolute flex justify-center align-end fill-screen" style="
            z-index: 9999;
            width: 100%;
            text-align: center;
            bottom: 64px;
        ...
        <div class="intro-container flex-center fill-screen" :style="`${clicked
            ? 'background: black'
            : hovering
              ? 'background: #BCC3C9 !important'
              : 'background: #CFD5DB !important'
          };`">
          <div v-if="(!isMobile && !unveil) || isMobile" class="veil absolute flex-center fill-screen" :style="isMobile &&
            ...
            <svg height="400" viewBox="0 0 1400 1000" fill="none">
              ...
              <path class="path3"
                ...
                stroke="#000000" stroke-width="200" />
            </svg>
          </div>
          <div v-if="!isMobile" class="zoomable">
            <div class="flex-center window" :class="clicked
                ? 'next window-hover'
                : boot || preboot
                  ? 'window-hover'
                  : 'window flex-center'
        ...
        <div class="absolute bg-image" :class="clicked ? 'desktop-modal-transition' : 'desktop-modal'"
          :style="`${hovering && 'animation-play-state: paused'};`">
          <!--GradientMesh :index="2" :style="`${popup && 'display: none;'}`" /-->
        </div>
        <div class="desktop-modal-final absolute bg-image" :style="desktopReady
            ? `
        opacity: 1;
        pointer-events: all;
